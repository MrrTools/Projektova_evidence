create or replace PACKAGE BODY pev_k_main AS
  ...
  vc_pkgname CONSTANT VARCHAR2(30 char) := 'pev_k_main';
...
/*!
  Procedura nastavi is_deleted z 0 na 1 a teda status bude pre zakaznika zmazany

  \p_id - id statusu
 */

PROCEDURE del_project_status(p_id in integer) AS
vc_procname VARCHAR2(100 char) := 'del_project_status';
BEGIN
     CEOS_LOG.set_context_func (vc_pkgname || '.' || vc_procname);
   CEOS_LOG.notice('Zmazanie ciselniku');
update PEV_T_LOV_PROJECT_STATUS set IS_DELETED = 1 where ID = p_id;
  ...
  WHEN OTHERS THEN
    ...
    RAISE;
END del_project_status;

----------------------------------------------------------------------------------

/*!
  Procedura nastavi is_deleted z 0 na 1 a teda stav bude pre zakaznika zmazany

  \p_id - id stavu
 */

PROCEDURE del_project_state(p_id in integer) AS
vc_procname VARCHAR2(100 char) := 'del_project_state';
BEGIN
     CEOS_LOG.set_context_func (vc_pkgname || '.' || vc_procname);
   CEOS_LOG.notice('Zmazanie ciselniku');
update PEV_T_LOV_PROJECT_STATE set IS_DELETED = 1 where ID = p_id;
  ...
  WHEN OTHERS THEN
    ceos_log.failure(SQLERRM || ' ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
    RAISE;
END del_project_state;

----------------------------------------------------------------------------------

/*!
  Procedura nastavi is_deleted z 0 na 1 a teda identifikace bude pre zakaznika zmazany

  \p_id - id identifikace
 */

PROCEDURE del_project_identification(p_id in integer) AS
vc_procname VARCHAR2(100 char) := 'del_project_identification';
BEGIN
     CEOS_LOG.set_context_func (vc_pkgname || '.' || vc_procname);
   CEOS_LOG.notice('Zmazanie ciselniku');
update PEV_T_LOV_PROJECT_IDENTIFY set IS_DELETED = 1 where ID = p_id;
  ...
  WHEN OTHERS THEN
    ceos_log.failure(SQLERRM || ' ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
    RAISE;
END del_project_identification;

----------------------------------------------------------------------------------

/*!
  Procedura nastavi is_deleted z 0 na 1 a teda pozicia bude pre zakaznika zmazany

  \p_id - id pozicie
 */

PROCEDURE del_people_position(p_id in integer) AS
vc_procname VARCHAR2(100 char) := 'del_people_position';
BEGIN
     CEOS_LOG.set_context_func (vc_pkgname || '.' || vc_procname);
   CEOS_LOG.notice('Zmazanie ciselniku');
update PEV_T_LOV_PEOPLE_POSITION set IS_DELETED = 1 where ID = p_id;
  ...
  WHEN OTHERS THEN
    ceos_log.failure(SQLERRM || ' ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
    RAISE;
END del_people_position;

----------------------------------------------------------------------------------

/*!
  Procedura nastavi is_deleted z 0 na 1 a teda project bude pre zakaznika zmazany

  \p_id - id projectu
 */

PROCEDURE del_projects(p_id in integer) AS
vc_procname VARCHAR2(100 char) := 'del_projects';
BEGIN
     CEOS_LOG.set_context_func (vc_pkgname || '.' || vc_procname);
   CEOS_LOG.notice('Zmazanie ciselniku');
...
IF p_email = 0 THEN
        ...
        --INSERT INTO PEV_T_CHANGES (RECORD_ID, CAT, CAT_ID, ITEM, OLD_VALUE, VALUE, ACTION) values (v_record_id, 'PROJECT', id_out,'Poznámka',p_old_note,p_note,p_action);
 ...
 */
  ...
  AS
    vc_procname VARCHAR2(100 char) := 'del_project_journal';
    p_del_date   DATE;
    p_del_text   VARCHAR2(2000 char);
    p_change_id  INTEGER;
  ...
  ceos_log.set_context_func (vc_pkgname || '.' || vc_procname);
  ceos_log.notice('Zmazanie zaznamu zo journalu');
  ...
  SELECT pev_s_changes_record.nextval INTO p_change_id FROM dual;
    ...
    INSERT INTO PEV_T_CHANGES
      ...
      values
        ...
        'JOURNAL',p_del_text,null,'D', p_del_date, null);


  --nastavenie priznaku na zmazany zaznam
  ...
  SELECT project_id INTO p_project_id FROM pev_t_project_logbook
  WHERE  record_id = p_id;
